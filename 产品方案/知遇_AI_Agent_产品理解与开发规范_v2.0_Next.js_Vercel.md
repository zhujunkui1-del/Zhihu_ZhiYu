...
>
> **v2.1 补记（2026-09，增量，不改动本文既有章节）**
>
> 前端视觉与交互已定稿，完整规格独立成文：**《视觉与交互规范.md》**。
> 本规范定义产品与技术，该文定义"长什么样、怎么交互"。两者冲突时，
> 产品定义以本规范为准，视觉与交互细节以该文为准。
>
> 该文补充了本规范尚未覆盖的四类内容：
>
> 1. **不在本规范中的新增能力**：「相遇雷达」——发现页的第二种结果视图，
>   以用户为中心、半径表示相似度、支持拖拽缩放、点头像开人格卡。
>   本规范 §7~§10 只定义了"快速匹配 / Agent 匹配"两级匹配，
>   雷达是其之上的**结果呈现层**，不改变两级匹配的产品定义。
> 2. **交互语义的细化**：「让我的 Agent 先聊聊」确定为**后台进行 + 原地留守**
>   （不跳转、弹提示、侧栏进度圆点）。本规范 §22 的流程图中
>   Agent 对话在后台进行这一层含义已明确到可实现的粒度。
> 3. **品牌资产与 IP 边界**：主视觉为知乎官方吉祥物**刘看山**，
>   仅用于本次黑客松赛事的演示与品牌致敬，不作商业用途；
>   商用替换点集中在 `assets/logo/` 与 `assets/characters/` 两组独立文件。
> 4. **设计 token 与组件规范**：暖调手绘纸感（主色暖珊瑚 `#e2664f`，
>   知乎品牌蓝仅留给授权入口）、非对称手绘圆角、纸纹底、组件样式表。
>   落地 `zhiyu-web/` 时可直接取该文 §2 作为 `globals.css` 的 `:root`。
>
> 该文另附「已知取舍与历史坑」一节（9 条），记录了实现阶段踩过的具体问题
> （派生色求值、列表盒内边距、弹窗内贴纯色底、指针捕获重定向点击目标等），
> 迁移到 React 时建议先读该节。
